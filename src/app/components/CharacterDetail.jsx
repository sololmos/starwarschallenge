...
const CharacterDetail = ({ characterId }) => {
  ...
  return (
    <div className="max-w-md my-10 mx-auto bg-white shadow-md rounded-md p-6">
      ...
      <div>
        <p><span className="font-bold">Eye Color:</span> {eye_color}</p>
        <p><span className="font-bold">Birth Year:</span> {birth_year}</p>
        <p><span className="font-bold">Hair Color:</span> {hair_color}</p>
        <p><span className="font-bold">Height:</span> {height}</p>
        <p><span className="font-bold">Skin Color:</span> {skin_color}</p>
        <p><span className="font-bold">Mass:</span> {mass}</p>
      </div>
    </div>
  );
};
...
//   const { id } = router.query; // Obtener el ID del personaje de los parámetros de la URL
